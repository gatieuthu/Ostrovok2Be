...
namespace Ostrovok2Be.RequestType
{
    ...
    {
        public static string getRateHotelInforByIds(string listIds,string checkin="",string checkout="", string lang = "en")
        {
            Debug.WriteLine(" GET RATE: " + listIds + " on Lang: " + lang+" CheckIn  "+checkin+  "  "+checkout);
            if (!string.IsNullOrEmpty(listIds))
            {
                System.Threading.Thread.Sleep(Form1.timeIdle);
                string api_getAllHotelByLocationText =
                    @"https://partner.ostrovok.ru/api/b2b/v2/hotel/rates?data={""ids"":[""" + listIds + @"""],""checkin"":"""+checkin+@""",""checkout"":"""+checkout+@""",""lang"":""" + lang + @"""}";
